...
help_msg() {
  ...
  echo "Usage:"
  echo ""
  echo "bash ./MCCE_bin/$(basename "$0") -h"
  echo "bash ./MCCE_bin/$(basename "$0") [env_name]"
  echo ""
...
MCCE_bin="$( dirname $( realpath "${BASH_SOURCE[0]}" ))"; 
...
then
  ...
  echo "NGPB image file already exists: No downloading."
...
then
  ...
  then
    ...
    # assign default name for latter use::
    env_name="mc4"
  else
    echo "Creating a dedicated conda env with user provided name: $env_name"
    conda env create -f "$REPO_PATH/mc4.yml" "$env_name"
  fi
fi
...
# Default rc file on Linux w/bash shell:
rc_file="$HOME/.bashrc"
...
then 
  ...
  if grep -q "$export1" "$rc_file";
  then
    echo "'MCCE4-Alpha/bin' already in PATH."
  fi
  if grep -q "$export2" "$rc_file";
  then
